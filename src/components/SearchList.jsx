...
function SearchList({ search, cities, setSearch, setSearchActive,setCities }) {
 ...
 const clearDefaults = () => {
    setCities([{lat:51.5072,lon:0.118092},{lat:50.4501,lon:30.5234},{lat:53.3498,lon:-6.2603},{lat:12.9716,lon:77.5946},{lat:25.2048,lon:55.2708}])
    setInfo([])
    setSearch("")
  ...
  return (
    <div className={styles.container}>
        <span onClick={clearDefaults} className={styles.backbtn}><MdArrowBack/></span>
      {info &&
        info.map((val, ind) => (
          <div
            onClick={() => setCoord(val.location)}
