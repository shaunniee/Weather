    ...
    <div className={styles.card}>
      ...
      <div className={styles.card__grid}>
        {forecast &&
          ...
          forecast.forecastday.map((val, ind) => (
            <div className={styles.card__item}>
              <span className={styles.card__item_time}>
                {day[new Date(val.date).getDay()]}
              </span>
              <span className={styles.card__item_icon}>
                <img
                  src={require(`../weather/64x64/day/${
                    json.find((X) => X.code === val.day.condition.code).icon
                  }.png`)}
                  alt="icon"
                />
                {}
              </span>
              <span className={styles.card__item_temp}>
                {`${val.day.maxtemp_c}/${val.day.mintemp_c}`} &deg; C
              </span>
            </div>
          ))}
      </div>
